    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise<void>((resolve) =>
          setTimeout(() => {
            console.log(
              "⏱️  ICE gathering timeout - proceeding with available candidates"
            );
            resolve();
          }, 3000)
        ),
  ...
  private async createPeerConnection() {
    ...
    this.peerConnection.addEventListener(
      ...
      (event: RTCIceCandidateEvent<"icecandidate">) => {
        if (event.candidate) {
          ...
          console.log(
            `   Type: ${candidateType}, Candidate: ${candidateStr.substring(
              0,
              50
            )}...`
          );
    ...
    this.peerConnection.addEventListener("icegatheringstatechange", () => {
      const gatheringState =
        this.peerConnection?.iceGatheringState || "unknown";
