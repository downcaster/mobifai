...
import {View, StyleSheet, LayoutChangeEvent, Text, GestureResponderEvent} from 'react-native';
...
function SliderComponent({
  ...
  const handleLayout = useCallback((event: LayoutChangeEvent): void => {
    const target = event.target as unknown as {
      measure: (
        callback: (x: number, y: number, width: number, height: number, pageX: number) => void,
      ) => void;
    };
  ...
  const handleTouch = useCallback(
    (evt: GestureResponderEvent): void => {
      ...
      <View
        ...
        {trackWidth > 0 && (
          <View
            ...
            <View style={[styles.thumbInner, {backgroundColor: activeTrackColor}]} />
          </View>
        )}
      </View>
