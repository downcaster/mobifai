...
export class CodeService {
  ...
  public getProjectsHistory(): Promise<CodeProject[]> {
    return new Promise((resolve, reject) => {
      ...
      const unsubscribe = this.onMessage(
        'code:projectsHistory',
        (_action, payload: ProjectsHistoryResponse) => {
          unsubscribe();
          resolve(payload.projects);
        },
      );

      const errorUnsubscribe = this.onMessage(
        'code:error',
        (_action, payload: CodeErrorResponse) => {
  ...
  public removeFromHistory(projectPath: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      const unsubscribe = this.onMessage(
        'code:projectRemovedFromHistory',
        (_action, payload: {projectPath: string}) => {
  ...
  public initProject(projectPath: string): Promise<ProjectInitializedResponse> {
    return new Promise((resolve, reject) => {
      const unsubscribe = this.onMessage(
        ...
        (_action, payload: ProjectInitializedResponse) => {
          unsubscribe();
          resolve(payload);
        },
      );
  ...
  public getFolderChildren(folderPath: string): Promise<FileNode[]> {
    return new Promise((resolve, reject) => {
      const unsubscribe = this.onMessage(
        'code:folderChildren',
        (_action, payload: FolderChildrenResponse) => {
  ...
  public getFile(filePath: string, projectPath?: string): Promise<string> {
    return new Promise((resolve, reject) => {
      const unsubscribe = this.onMessage(
        'code:fileContent',
        (_action, payload: FileContentResponse) => {
      ...
      const payload: CodeFilePayload & {projectPath?: string} = {
        filePath,
        projectPath,
      };
  ...
  public saveFile(filePath: string, newContent: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      const unsubscribe = this.onMessage(
        'code:fileSaved',
        (_action, payload: FileSavedResponse) => {
          if (payload.filePath === filePath) {
            unsubscribe();
      ...
      const errorUnsubscribe = this.onMessage(
        'code:fileSaveError',
        (_action, payload: FileSaveErrorResponse) => {
          if (payload.filePath === filePath) {
            errorUnsubscribe();
            reject(new Error(payload.error));
          }
  ...
  public createFile(
    folderPath: string,
    fileName: string,
  ): Promise<{filePath: string; parentFolder: string; children: FileNode[]}> {
    return new Promise((resolve, reject) => {
      const unsubscribe = this.onMessage(
        'code:fileCreated',
        (
      ...
      const errorUnsubscribe = this.onMessage(
        ...
        (_action, payload: {folderPath: string; fileName: string; error: string}) => {
  ...
  public createFolder(
    ...
    return new Promise((resolve, reject) => {
      ...
      const errorUnsubscribe = this.onMessage(
        'code:createFolderError',
        (_action, payload: {parentPath: string; folderName: string; error: string}) => {
  ...
  public renameItem(
    ...
    return new Promise((resolve, reject) => {
      ...
      const errorUnsubscribe = this.onMessage(
        'code:renameError',
        (_action, payload: {path: string; error: string}) => {
   ...
   */
  public deleteItem(itemPath: string): Promise<{
    deletedPath: string;
    parentFolder: string;
    children: FileNode[];
  }> {
  ...
  public getProjectChanges(projectPath: string): Promise<{
    ...
    return new Promise((resolve, reject) => {
      ...
      const errorUnsubscribe = this.onMessage(
        'code:projectChangesError',
        (_action, payload: {projectPath: string; error: string}) => {
          if (payload.projectPath === projectPath) {
            errorUnsubscribe();
            reject(new Error(payload.error));
          }
  ...
  public openFile(
    ...
    return new Promise((resolve, reject) => {
      ...
      const errorUnsubscribe = this.onMessage(
        'code:fileOpenError',
        (_action, payload: {filePath: string; error: string}) => {
          if (payload.filePath === filePath) {
            errorUnsubscribe();
            reject(new Error(payload.error));
          }
  ...
  public closeFile(filePath: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const unsubscribe = this.onMessage(
        'code:fileClosed',
        (_action, payload: {filePath: string}) => {
          if (payload.filePath === filePath) {
            unsubscribe();
            resolve();
          }
  ...
  ): Promise<{filePath: string | null; content: string | null}> {
    return new Promise((resolve, reject) => {
      const unsubscribe = this.onMessage(
        'code:activeFileSet',
        (_action, payload: {filePath: string | null; content: string | null}) => {
          if (payload.filePath === filePath) {
            unsubscribe();
            resolve(payload);
          }
      ...
      const errorUnsubscribe = this.onMessage(
        'code:activeFileError',
        (_action, payload: {filePath: string | null; error: string}) => {
          if (payload.filePath === filePath) {
            errorUnsubscribe();
            reject(new Error(payload.error));
          }
